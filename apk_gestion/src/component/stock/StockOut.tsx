...
function StockOut() {
    ...
    useEffect(() => {
        axios.get('http://localhost:3001/gestionDB/stock/out')
          .then(response => {
            if (response.data) {
                // Filtrer les produits qui ont une quantité de sortie > 0
                const filteredStocks = response.data.map(stock => ({
                    ...stock,
                    produits: stock.produits.filter(produit => produit.quantite_sortie > 0)
                    ...
                    <main style={{ flexGrow: 1, padding: '10px' }}>
                        ...
                        <Paper>
                            <Typography variant="h3" style={{ fontWeight: 'bold', color: '#3f51b5', textAlign: 'center', margin: '20px 0' }}>
                                Mouvements de Stock et Quantité Actuelle (Sortie)
                            </Typography><br />
                            <Table>
                                <TableHead>
                                    <TableRow sx={{ backgroundColor: 'blue' }}>
                                        ...
                                        <TableCell sx={{color: 'white' , fontSize: '1rem' , fontWeight: 'bold'}}>Quantité Sortie</TableCell>
                                            ...
                                            <TableRow key={`${stock._id}-${index}`} sx={{ 
                                                ...
                                                {index === 0 && (
                                                    <TableCell rowSpan={stock.produits.length} sx={{ borderRight: '3px solid #f9f9f9' }}>
                                                        {new Date(stock.date_sortie).toLocaleDateString()}
                                                    </TableCell>
                                                )}
                                                <TableCell>{produit.produit_id?.label || 'Non spécifié'}</TableCell> 
                                                <TableCell>{produit.quantite_sortie}</TableCell>
